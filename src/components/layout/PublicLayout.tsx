    ...
    <div className="min-h-screen flex flex-col bg-zinc-950 text-zinc-50 border-border">
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-8">
              ...
              <div className="hidden md:flex items-center space-x-6 text-sm font-medium">
                <Link to="/" className={`${location.pathname === '/' ? 'text-white' : 'text-zinc-400 hover:text-white transition-colors'}`}>{translateUI('nav.home')}</Link>
                <Link to="/schedule" className={`${location.pathname.startsWith('/schedule') ? 'text-white' : 'text-zinc-400 hover:text-white transition-colors'}`}>{translateUI('nav.schedule')}</Link>
                <Link to="/shows" className={`${location.pathname.startsWith('/shows') ? 'text-white' : 'text-zinc-400 hover:text-white transition-colors'}`}>{translateUI('nav.shows')}</Link>
                <Link to="/news" className={`${location.pathname.startsWith('/news') ? 'text-white' : 'text-zinc-400 hover:text-white transition-colors'}`}>{translateUI('nav.news')}</Link>
      ...
      <footer className="py-12 bg-zinc-950 border-t border-zinc-900 text-center text-zinc-500">
        ...
        <div className="flex justify-center flex-wrap gap-x-6 gap-y-3 text-sm mb-6 font-medium">
          ...
          <Link to="/shows" className="hover:text-zinc-300">{translateUI('nav.shows')}</Link>
          <Link to="/news" className="hover:text-zinc-300">{translateUI('nav.news')}</Link>
